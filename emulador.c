...
int main()
{
    CPU cpu;           // Crear instancia de CPU
    resetCPU(&cpu);    // Inicializar a estado conocido

    // Programa de ejemplo para ejecutar
    uint16_t programa[] = {
        0x24A,  // Instrucción 1: LD X, [0x0A] - Carga X con valor en mem[0x0A]
        0x44B,  // Instrucción 2: ST X, [0x0B] - Almacena X en mem[0x0B]  
        0x04C,  // Instrucción 3: ADD ACC, [0x0C] - Suma mem[0x0C] a ACC
        0xE00,  // Instrucción 4: HALT - Detiene la CPU
        
        // Memoria intermedia (direcciones 4-9)
        0x000, 0x000, 0x000, 0x000, 0x000, 0x000,
        
        // Datos en addresses 0x0A-0x0C  
        0x0005,  // mem[0x0A] = 5 - Dato para LD
        0x0006,  // mem[0x0B] = 6 - Dato para ST
        0x0002   // mem[0x0C] = 2 - Dato para ADD
    };
    
    // Cargar programa en memoria de la CPU
    for (int i = 0; i < (sizeof(programa)/sizeof(uint16_t)); i++) {
        cpu.mem[i] = programa[i];
    }

    // Inicializar registros con valores de prueba
    cpu.acc = 0x10;  // Acumulador = 16 decimal
    cpu.x = 0x20;    // Registro índice = 32 decimal

    // Iniciar emulación
    printf("Starting CPU emulation...\n");
    cpu_loop(&cpu);  // Ejecutar hasta HALT
